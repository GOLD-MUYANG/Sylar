#ifndef __SYLAR__HTTP_HTTP_H__
#define __SYLAR__HTTP_HTTP_H__
...
private:
    // 检查map中是否有key并且返回对应的bool，，同时传入一个参数，有则转换为T类型，没有则返回def,
    //其实这种引用传递就做了多返回值的功能了
    template <class T>
    T checkGetAs(const MapType &map, const std::string &key, T &val, const T &def = T())
    {
