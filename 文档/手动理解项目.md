...
项目对 HTTP 协议的处理主要涉及以下四部分：

1. HTTP 数据结构。
2. HTTP 报文解析。
3. 在 Socket 上收发 HTTP 报文。
4. HTTP 服务端处理循环。
...
```text
HttpConnection::recvResponse()
    ↓
HttpResponseParser
    ↓
httpclient_parser 状态机
    ↓
生成 HttpResponse
```

## 7.2 HTTP 数据结构：`http.cc`

`http.cc`（第 62 行附近）定义了项目内部对 HTTP 的统一表示，主要包括：

| 类型 / 方法 | 作用 |
| --- | --- |
| `HttpMethod` | 表示 `GET`、`POST`、`PUT`、`DELETE` 等请求方法 |
| `HttpStatus` | 表示 `200`、`404`、`500` 等响应状态码 |
| `HttpRequest` | 保存 method、path、query、fragment、headers、body、params、cookies、HTTP version 和是否关闭连接等请求信息 |
| `HttpResponse` | 保存 status、reason、headers、body、HTTP version 和是否关闭连接等响应信息 |
| `CaseInsensitiveLess` | 让请求头名称不区分大小写 |
| `dump()` / `toString()` | 把请求或响应对象重新序列化成 HTTP 文本报文 |

## 7.3 HTTP 报文解析

### 请求解析：`http11_parser.rl`

`http11_parser.rl` 是 Ragel 状态机的源文件，定义了：

- 请求方法如何识别。
- URI、path、query、fragment 如何拆分。
- HTTP/1.0、HTTP/1.1 如何识别。
- Header 名称和值如何解析。
- 什么时候认为请求头已经完整。

在需要使用这部分代码时，项目会自动执行 Ragel 命令生成对应的 C 文件，用于解析
HTTP 协议。

### 响应解析：`httpclient_parser*`

`httpclient_parser*` 用于解析客户端收到的响应头。它与 `http11_parser.rl` 相对应：
一个负责解析请求，一个负责解析响应。

两者不能直接复用，是因为请求和响应的第一行完全不同：

```http
请求：POST /path HTTP/1.1
响应：HTTP/1.1 200 OK
```

### 解析入口：`http_parser.cc`

`http_parser.cc` 调用 `http11_parser.rl` 生成的状态机代码解析 HTTP 请求。

`HttpRequestParser` 初始化时会初始化状态机：

```cpp
http_parser_init(&m_parser);
```

`HttpSession` 从 Socket 读到数据以后调用：

```cpp
size_t nparse = parser->execute(data, len);
```

随后进入状态机的 `http_parser_execute`。当状态机解析出请求方法、URI 等字段后，
会调用 sylar 中定义的回调函数，把解析结果设置到 `HttpRequest` 对象中。

## 7.4 状态机如何写入 `HttpRequest`
...
接着进入 `on_request_method()`：

```cpp
void on_request_method(void *data, const char *at, size_t length)
{
    HttpRequestParser *parser =
        static_cast<HttpRequestParser *>(data);
...
```cpp
m_data->setMethod(HttpMethod::GET);
```
...
Ragel 状态机识别字段
    ↓
调用 HttpRequestParser 中注册的回调函数
    ↓
回调函数把字段写入 HttpRequest
```

## 7.5 `HttpSession` 如何接收请求

`http_session.cc`（第 37 行附近）中的 `HttpSession` 继承自 `SocketStream`，代表：

> 服务端已经接受的一条 TCP 连接，以及在这条连接上收发 HTTP 报文的能力。

`recvRequest()` 的主要流程是：

1. 从 Socket 读取一部分字节。
2. 把数据交给 `HttpRequestParser`。
3. 如果请求头不完整，则继续读取。
4. 请求头完整后，取得 `Content-Length`。
5. 保存已经提前读到的 body。
6. 如果 body 不完整，则继续通过 `readFixSize()` 读取。
7. 最后返回 `HttpRequest`。
...
```text
                   继承
Stream  <──  SocketStream  <──  HttpSession
                │
                │ 组合：m_socket
                ▼
              Socket
...
| 组件 | 职责 |
| --- | --- |
| `Socket` | 负责网络资源、连接、地址、超时和 `send` / `recv` |
| `SocketStream` | 把 `Socket` 适配成统一的 `Stream` 读写接口 |
| `Stream` | 提供与底层介质无关的固定长度读写算法 |
| `HttpSession` | 在 `Stream` 上增加 HTTP 请求解析和响应发送能力 |

### 一次请求的读取过程

接收到请求后，`HttpSession::recvRequest()` 会先读取一批数据，再交给 Ragel 生成的代码
解析。当状态机识别到 Header 后面的空行 `\r\n\r\n` 时，请求头解析完成。

取得请求头中的 `Content-Length` 后，就能确定 body 的长度。由于单次 `read` 不一定能
拿到全部数据，因此需要持续读取，直到达到 body 长度。`Stream` 负责控制固定长度读取循环，
`SocketStream` 负责执行单次实际读取，并转发给 `Socket::recv()`。

### 一句话理解

`HttpSession` 负责把 Socket 字节流读完整，`HttpRequestParser` 负责把字节解析成
`HttpRequest`，Ragel 状态机则负责识别报文中的具体字段。
...
process_worker 调度连接处理协程
  ▼
重写方法，HttpServer::handleClient()
  ▼
HttpSession::recvRequest()，确保读取完整一个 HTTP 请求报文
  ▼
HttpRequest 将请求报文解析成 HttpRequest（请求行，请求头，请求报文）对象
  ▼
HttpRequestParser，对一批数据进行解析，解析出一个完整字段后调用回调，设置 HttpRequest 对象的对应字段
  ▼
ServletDispatch 按 HttpRequest 中的 path 查找 Servlet
  ▼
AiGatewayServlet::handle()
  ▼
