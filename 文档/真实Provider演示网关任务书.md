...
> 状态：D0-D6 已完成，D7 已完成手工入口与默认拒绝边界；真实 Provider live 成功请求仍需要操作者在本机显式提供 API Key 后验证。本文承接 `文档/真实提供商网关.md` 已完成的真实 Provider 基础能力，把它推进到“可配置、可真实聊天、可观察、可演示”的运行链路。默认测试和 CI 仍不依赖真实 API Key、外网或计费账户。
...
当前已落地到 `modules/ai_gateway/real_provider_runtime.{h,cc}`：

- `LoadRealProviderRuntimeConfig()` 解析独立 `real_providers` 节点；
- `enabled=false` 时不读取 Key、不访问外网；
- `enabled=true` 时只按 `api_key_env` 读取环境变量，缺 Key 返回脱敏配置错误；
- enabled provider 会校验 `base_url`、`chat_path`、`logical_model`、`upstream_model`、`compatibility_key` 和 `api_key_env`；
- provider 级 TLS 元数据写入 `ProviderCandidate`，由 OpenAI-compatible adapter 传给 `HttpRequestOptions`。
...
当前实现通过 `RealProviderRuntime` 复用 `LogicalModelRouter`、`ProviderAttemptExecutor` 与 `CreateOpenAICompatibleAttemptHandler()`，并在 `ai_gateway_module.cc` 中仅当 `real_providers.enabled=true` 时接入 `/v1/chat/completions`。
...
当前 `AiGatewayServlet` 已保留原 mock `UpstreamPost`，并新增 `CompatibleUpstreamPost`。真实 Provider 路径传入解析后的 `ChatCompletionRequest`，返回 executor 已构造好的 OpenAI-compatible 响应；servlet 不拼接真实 Provider URL、header 或 body。
...
| 切片 | 状态 | 目标 | 验证 |
| --- | --- | --- | --- |
| D0 | 已完成 | 任务书落地，明确真实演示链路边界 | 本文档继续把真实 Provider 访问默认关闭，并与 `文档/真实提供商网关.md` 的 R0-R7 基础闭环分层 |
| D1 | 已完成 | 新增真实 Provider yml 配置读取，支持多个 Provider 和 env-only API Key | `test_ai_gateway_real_runtime` 覆盖 enabled provider、缺 Key 脱敏失败、`compatibility_key` 经 router 校验 |
| D2 | 已完成 | 把真实 Provider executor 接入 `/v1/chat/completions`，但保持显式开关 | `test_ai_gateway_real_runtime` fake transport 验证真实路径响应成功；`test_ai_gateway_servlet` 验证 servlet 真实回调返回兼容响应 |
| D3 | 已完成 | 扩展真实 Provider trace，覆盖请求进入、解析、候选、尝试、决策、返回 | `test_ai_gateway_real_runtime` 与 `test_ai_gateway_servlet` 检查 trace 不含 Key/messages |
| D4 | 已完成 | 扩展 `/internal/status` 或新增内部状态接口展示真实 Provider 状态 | `test_ai_gateway_status` 覆盖 `real_providers` 状态段和脱敏输出；`RealProviderRuntime::buildStatusJson()` 输出 provider 计数、breaker、in-flight |
| D5 | 已完成 | 新增演示页面：左侧聊天，右侧 Provider 状态和 timeline | `/demo` 已兼容 mock `attempts` 与真实 provider `events`，状态区优先展示 `real_providers.providers` |
| D6 | 已完成 | 新增演示启动器或脚本，启动真实 Provider 页面演示 | `scripts/demo_ai_gateway_real_provider.sh` 先做 opt-in/key 可见性检查，然后直接使用 `examples/ai_gateway_conf/ai_gateway.yml`；provider 列表完全以 yml 为准，不补默认 provider |
| D7 | 已完成手工入口；待本机 Key live 成功验证 | 真实 Provider live smoke 接入页面或脚本入口，仍保持手工 opt-in | `scripts/smoke_ai_gateway_real_provider.sh` 与 `scripts/demo_ai_gateway_real_provider.sh` 默认/缺 Key 不触网；设置 Key 后需操作者手工验证真实请求成功 |
...
真实 Provider 页面演示必须手工 opt-in，且不会把临时配置写回仓库：

```bash
# 按 examples/ai_gateway_conf/ai_gateway.yml 中 enabled provider 的 api_key_env 设置。
...
该脚本完全以 `examples/ai_gateway_conf/ai_gateway.yml` 为准，不补默认 provider，也不生成临时 provider 配置；yml 中 enabled provider 写了哪些 `api_key_env`，当前 shell 就需要能看到哪些变量。若只想演示某一个真实 Provider，应先在 yml 中把其它 provider 的 `enabled` 改为 `false`。若请求已发送后等待响应超时，网关仍会返回结果未知，不会盲目切换到 mock provider。
